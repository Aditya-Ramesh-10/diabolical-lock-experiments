...
from db_lock_models import RandomFeatureNetwork, EnsembledFeatureNetwork
from envs.diabolical_combination_locks import HighDimensionalDiabolicalCombinationLock
from agents.ppo_rcgvf import PPOWithRCGVF
...
config_defaults = dict(
    ...
    optim_alpha=0.99,
    text=False,
    intrinsic_coef=20.0, ## Can be set to 2.0 when use_std is True
    lr_predictor_factor=0.5,
    ...
    gvf_lambda=0.9,
    use_std=False,
    predictor_hidden_dim_size=256,
    shared_ac_architecture=True,
    record_lock_completion=True,
    # evaluation ---------------------------
    eval_every_interval=200,  # zero means no separate evaluation
    eval_episodes=10,
    greedy_eval=True,
)
...
def main():
    ...
    predictor_network = EnsembledFeatureNetwork(obs_space,
                                                args.rnd_embedding_size,
                                                args.predictor_hidden_dim_size,
                                                n_ensemble=args.n_heads_ensemble,
    ...
    elif args.algo == "ppo_rcgvf":

        algo = PPOWithRCGVF(envs=envs, acmodel=acmodel, device=device, num_frames_per_proc=args.frames_per_proc, 
                          ...
                          lr_predictor=args.lr_predictor_factor * args.lr, gvf_discount=args.gvf_discount,
                          gvf_lambda=args.gvf_lambda, use_std=args.use_std)
