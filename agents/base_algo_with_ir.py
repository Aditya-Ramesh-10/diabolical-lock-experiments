from utils.par_env import ParallelEnv
...
class BaseAlgoWithIR(ABC):
    ...
    def __init__(self, envs, acmodel, device, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
        ...
        record_lock_completion : bool
            whether to record the completion of the lock in the environment
        """
        ...
        # Diagnostics
        self.record_lock_completion = record_lock_completion
        if self.record_lock_completion:
            self.farthest_column_visited = 0
    ...
    def collect_experiences(self):
        ...
        intrinsic_rewards = self.compute_intrinsic_rewards(preprocessed_obs)
        ...
        if self.record_lock_completion:
            logs["farthest_column_visited"] = self.farthest_column_visited

        logs = self._append_logs_collect(logs)
    ...
    def update_completion_info(self):
        agent_states = self.env.state_extraction_key()
        for k in range(len(agent_states)):
            row_num, col_num = agent_states[k]
            if (row_num < 2) and (col_num > self.farthest_column_visited):
                self.farthest_column_visited = col_num